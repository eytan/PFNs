...
Code adapted from https://github.com/boschresearch/Scalable-Meta-Learning-with-Gaussian-Processes/blob/main/scamlgp/model.py"""
...
def _get_default_likelihood_noise(
...
def _get_default_likelihood(
    ...
    return GaussianLikelihood(
        # noise_prior=noise_prior,
        noise_constraint=Interval(1e-8, 1e-5, initial_value=1e-6),
        batch_shape=batch_shape,
    )
...
def _compute_target_prior(
    ...
    prior_mean_target = sum(weighted_post_mean_source, torch.zeros(*x.shape[:-1]).to(x))
...
def meta_fit_scamlgp(
    ...
    for task_id, task_data in meta_data.items():
        ...
        task_mll = ExactMarginalLogLikelihood(task_gp.likelihood, task_gp)

        fit_gpytorch_mll(
            task_mll,
            num_restarts=num_restarts_log_likelihood,
...
class ScaMLGP(SingleTaskGP):
    ...
    def forward(self, x: torch.Tensor) -> MultivariateNormal:
        ...
        # print(mean.squeeze(-1).shape, cov.to_dense().shape)#,squeeze(-1), cov.to_dense())
        # if mean.shape[0] == 1:
        #     return torch.distributions.Normal(mean.squeeze(-1), cov)
